...
order_hours = instacart %>% arrange(order_hour_of_day) %>% distinct(order_hour_of_day) %>% pull()

# selectInput widget
selectInput("order_hour_choice", label = h3("Select the order hour of day"),
            choices = order_hours)
...
renderPlotly({
  aisle_tidy_data %>%
    filter(order_hour_of_day == input$order_hour_choice) %>% 
